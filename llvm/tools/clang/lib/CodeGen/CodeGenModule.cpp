...
//Aggiunto DFA
  if (D->hasAttr<SBoxAttr>()) {
    llvm::NamedMDNode *MD = getModule().getOrInsertNamedMetadata("sbox.vars");
    MD->addOperand(llvm::MDNode::get(getLLVMContext(), GV));
  }

 if (D->hasAttr<MaskedCopyAttr>()){
 llvm::AttrBuilder B; 
 B.addAttribute(llvm::Attribute::MaskedCopy);
