...
void CodeGenFunction::GenerateCode(GlobalDecl GD, llvm::Function *Fn,
  ...
  if(FD->getNumParams()>0){
        ...
        for(int i=0;i< FD->getNumParams() && ai!=ae;++ai,i++){
        auto d = FD->getParamDecl(i);
                if(d->getAttr<KeyAttr>()){
                        llvm::Value *var = ai;
                        llvm::Value *V = llvm::MDNode::get(C,var);
                        llvm::Value *F = CGM.getIntrinsic(llvm::Intrinsic::crypto_key);
                        Builder.CreateCall(F,V);
                }else
                if(d->getAttr<PlainAttr>()){
                        llvm::Value *var = ai;
                        llvm::Value *V = llvm::MDNode::get(C,var);
                        llvm::Value *F = CGM.getIntrinsic(llvm::Intrinsic::crypto_plain);
                        Builder.CreateCall(F,V);
                }
         }
  }
