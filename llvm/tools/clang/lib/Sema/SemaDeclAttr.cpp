...
//Aggiunto DFA
static void handlePlain(Sema &S, Decl *d, const AttributeList &Attr){
        if(!isa<VarDecl>(d)){
                S.Diag(Attr.getLoc(),diag::err_attribute_wrong_decl_type) << Attr.getName() << 8;
                return;
        }
        d->addAttr(::new (S.Context) PlainAttr(Attr.getRange(),S.Context));
}
static void handleKey(Sema &S, Decl *d, const AttributeList &Attr){
        if(!isa<VarDecl>(d)){
                S.Diag(Attr.getLoc(),diag::err_attribute_wrong_decl_type) << Attr.getName() << 8;
                return;
        }
...
static void handleMaskedCopyAttr(Sema &S, Decl *D, const AttributeList &Attr) {
  ...
  if (!isa<FunctionDecl>(D)) {
    S.Diag(Attr.getLoc(), diag::warn_attribute_wrong_decl_type)  << Attr.getName() << ExpectedFunction;
    return;
  }
